...
export function up(next: () => void): void {
  db.exec(`
    BEGIN TRANSACTION;
    ALTER TABLE nodes ADD COLUMN downloadStatus TEXT CHECK( downloadStatus IN ('downloading', 'downloadingPause', 'verifying','verifyingPause', 'extracting','extractingPause', 'finish') ) NOT NULL DEFAULT 'finish';
