import { app, shell, BrowserWindow, Tray, Menu, ipcMain } from 'electron'
import { join } from 'path'
import { electronApp, optimizer, is } from '@electron-toolkit/utils'
import icon from '../../resources/app/iconTemplate.png?asset'
import trayIcon from '../../resources/tray/iconTemplate.png?asset'
import * as gwat from './node/gwat'
import * as coordinatorBeacon from './node/coordinator-beacon'
import { getPlatform } from './utils'

let tray
let mainWindow

async function nodeStart(): Promise<string> {
  console.log('start')
  if (coordinatorBeacon.isRunning()) {
    await coordinatorBeacon.stop()
  }
  if (gwat.isRunning()) {
    await gwat.stop()
  }
  const respGwat = await gwat.start()
  console.log(`start gwat: ${respGwat}`)
  const respCoord = await coordinatorBeacon.start()
  console.log(`start coord: ${respCoord}`)
  return `start: ${respGwat}/${respCoord}`
}

async function nodeStop(): Promise<string> {
  console.log('stop')
  if (!coordinatorBeacon.isRunning()) {
    console.log('stop coord: not running')
  } else {
    const resp = await coordinatorBeacon.stop()
    console.log(`stop coord: ${resp}`)
  }

  if (!gwat.isRunning()) {
    console.log('stop gwat: not running')
  } else {
    const resp = await gwat.stop()
    console.log(`stop gwat: ${resp}`)
  }

  return `stop`
}


function createWindow(): void {
  // Create the browser window.
  mainWindow = new BrowserWindow({
    width: 900,
    height: 670,
    show: false,
    autoHideMenuBar: true,
    icon: icon,
    title: 'Waterfall App',
    titleBarStyle: 'hidden',
    trafficLightPosition: { x: 10, y: 12 },
    webPreferences: {
      preload: join(__dirname, '../preload/index.js'),
      sandbox: false
    }
  })

  mainWindow.on('ready-to-show', () => {
    mainWindow.show()
  })

  mainWindow.on('close', function(event): void {
    event.preventDefault()
    mainWindow.hide()
  })

  mainWindow.webContents.setWindowOpenHandler((details) => {
    shell.openExternal(details.url)
    return { action: 'deny' }
  })

  // HMR for renderer base on electron-vite cli.
  // Load the remote URL for development or the local html file for production.
  if (is.dev && process.env['ELECTRON_RENDERER_URL']) {
    mainWindow.loadURL(process.env['ELECTRON_RENDERER_URL'])
  } else {
    mainWindow.loadFile(join(__dirname, '../renderer/index.html'))
  }
}

app.setName('Waterfall App')
// This method will be called when Electron has finished
// initialization and is ready to create browser windows.
// Some APIs can only be used after this event occurs.
app.whenReady().then(() => {
  // Set app user model id for windows
  electronApp.setAppUserModelId('app.waterfall')

  // Default open or close DevTools by F12 in development
  // and ignore CommandOrControl + R in production.
  // see https://github.com/alex8088/electron-toolkit/tree/master/packages/utils
  app.on('browser-window-created', (_, window) => {
    optimizer.watchWindowShortcuts(window)
  })

  console.log(app.getName())

  // const trayIcon = nativeImage.createFromDataURL('data:image/png;base64,')
  // const trayIcon = nativeImage.createFromPath('../../resources/icon2.png')
  tray = new Tray(trayIcon)
  const contextMenu = Menu.buildFromTemplate([
    {
      label: 'Show App',
      click: (): void => {
        // if (getPlatform() === 'mac') {
        //   app.show()
        // }
        // app.focus()
        mainWindow.show()
        console.log('Show')

      }
    },
    {
      label: 'Quit',
      click: (): void => {
        mainWindow.destroy()
        app.quit()
        console.log('Quit')
      }
    }
  ])

  tray.setContextMenu(contextMenu)
  tray.setToolTip('Waterfall App')

  if (getPlatform() === 'mac') {
  //   app.dock.hide()
    app.dock.setIcon(icon)
  }


  ipcMain.handle('node:start', nodeStart)
  ipcMain.handle('node:stop', nodeStop)
  createWindow()

  app.on('activate', function() {
    // On macOS it's common to re-create a window in the app when the
    // dock icon is clicked and there are no other windows open.
    if (BrowserWindow.getAllWindows().length === 0) createWindow()
  })
})

// Quit when all windows are closed, except on macOS. There, it's common
// for applications and their menu bar to stay active until the user quits
// explicitly with Cmd + Q.
app.on('window-all-closed', () => {
  // if (process.platform !== 'darwin') {
  //   app.quit()
  // }
})

// In this file you can include the rest of your app"s specific main process
// code. You can also put them in separate files and require them here.
